...
def my_galleries():
    ...
    return render_template("my_galleries.html", side_nav_elements=result, current_path=current_path, galleries=galleries)
...
def create_new_gallery():
	...
	if request.method == "POST":
		...
		new_gallery = Gallery(
			...
			tags = StringHelper.parse_tags_from_text(request.form.get("gallery-tags", ''), app.logger),
		...
		new_gallery.set_file_paths(base_path, request.files.getlist("gallery-images"))

		# Save to disk
		GalleryHelper.save_to_disk(base_path, new_gallery, request.files.getlist("gallery-images"), app.logger)
